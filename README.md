...
rustblocker --https --https-port 443            # Enable HTTPS (requires certificate in DB)
rustblocker --https --https-port 8443           # HTTPS on high port (no root needed)
...
```bash
# Production (requires root for port 443)
sudo rustblocker --https --https-port 443

# Development (high port, no root needed)
rustblocker --https --https-port 8443

# HTTP-only (ignores any HTTPS configuration)
rustblocker --force-http
```

When `--https` is set, RustBlocker loads the certificate from the database and binds both HTTP and HTTPS. If no valid certificate is found, it falls back to HTTP-only with a warning.
